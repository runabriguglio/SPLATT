    ...
    def startGui(self):
        ...
        def _formatGraphics(gui, *args):
            for ch in range (3):
                ...
                cguis[ch].V = 'null'
                cguis[ch].A = 'null'
                cguis[ch].widgets['V'].setContentsMargins(10,0,10,0)
                cguis[ch].widgets['A'].setContentsMargins(10,0,10,0)
...
###################################################################
        ...
        def _monitoring(gui, *args):
            ...
            for ch in range(1,4):
                if self.ch[ch-1]:
                    voltage = f"{self.power_supply.read_voltage(ch):.2f} V"
                    current = f"{self.power_supply.read_current(ch):.2f} A"
                else:
                    voltage = 'null'
                    current = 'null'
                cguis[ch].V = voltage
