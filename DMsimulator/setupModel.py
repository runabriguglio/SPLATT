...
# write_to_fits(fit_cube_err, 'FitErrData.fits')
plt.figure()
plt.plot(rel_rms,'o')
plt.grid('on')
...
#     interpolator = tps.fit(real_pix_coords, act_data)
...
for kk in range(n_hex):
    glob_img[dm.hex_valid_ids[kk]] = hexA.IFF[:,kk]#loc_IFF[:,kk]
